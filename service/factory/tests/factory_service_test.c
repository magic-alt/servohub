...
int main(void)
{
    ...
    source.iq_ref_a = 1.0F; source.iq_fb_a = 0.8F; FactoryServiceTracePushFocIsr(&source);
    assert(FactoryServiceTraceManualTrigger() == FACTORY_TRACE_OK);
    source.iq_ref_a = 2.0F; source.iq_fb_a = 1.7F; FactoryServiceTracePushFocIsr(&source);
    source.iq_fb_a = 1.9F; FactoryServiceTracePushFocIsr(&source);
    ...
    assert(FactoryServiceTraceReadSample(1U, &sample) == FACTORY_TRACE_OK);
...

    {
        ...
        assert(FactoryServiceReadDiagnostic(FACTORY_DIAG_POWER_DC_BUS_VOLTAGE, &value) == FACTORY_TRACE_OK);
