...
void app_init(void)
{
    ...
    {
        app_info[i].app_ptr = &app_table[i];   // 设置应用程序指针
        app_info[i].enable = false;            // 禁用应用程序
        app_info[i].app_state = APP_STA_IDLE;  // 设置应用程序状态为空闲
        app_info[i].app_result = APP_RET_IDLE; // 设置应用程序结果为空闲
    }
}
        ...
        if (app_table[i].name == name)
        {
            ...
            if (app_table[i].type == APP_TYPE_PDS)
            {
                for (int j = 0; j < APP_NUM; j++)
                {
                    ...
                    {
                        if (app_info[j].app_state != APP_STA_IDLE)
                        {
                            // 若有PDS类型的应用程序正在运行，则停止它
                            stop_app(app_table[j].name);
                        }
                    }
                }
            }
            ...
            {
                // 如果当前应用程序正在运行，则停止它
                if (app_info[i].app_state != APP_STA_IDLE)
                {
                    stop_app(app_table[i].name);
                }
            }

            // 初始化应用程序
            if (app_info[i].app_ptr->init != NULL)
            {
                if (app_info[i].app_ptr->init() == APP_RET_SUCCESS)
                {
                    app_info[i].app_state = APP_STA_READY;
                }
                else
                {
                    app_info[i].app_state = APP_STA_ERROR;
                }
            }

            app_info[i].enable = true;             // 启用应用程序
            app_info[i].app_result = APP_RET_IDLE; // 设置应用程序结果为空闲
            break;
        }
    }
...
void app_run(uint8_t hw_ready_state)
{
    ...
    for (int i = 0; i < APP_NUM; i++)
    {
        if (app_info[i].enable)
        {

            // 启动应用程序
            if (app_info[i].app_state == APP_STA_READY)
            {

                if (app_info[i].app_ptr->start != NULL)
                {
                    app_info[i].app_result = app_info[i].app_ptr->start();
                    if (app_info[i].app_result == APP_RET_RUNNING)
                    { // start处于等待状态
                        app_info[i].app_state = APP_STA_READY;
                    }
                    else if (app_info[i].app_result == APP_RET_SUCCESS)
                    { // start成功,进入run状态
                        app_info[i].app_state = APP_STA_RUNNING;
                    }
                    else if (app_info[i].app_result == APP_RET_FAIL)
                    { // start失败,进入错误状态
                        app_info[i].app_state = APP_STA_ERROR;
                    }
                }
            ...
            // 运行应用程序
            if (app_info[i].app_state == APP_STA_RUNNING)
            {
                if (app_info[i].app_ptr->run != NULL)
                {
                    // 回读PWM准备好状态，准备好则运行APP 否则等待处于 READY 状态
                    if (hw_ready_state)
                    {
                        app_info[i].app_result = app_info[i].app_ptr->run();
                        if (app_info[i].app_result == APP_RET_RUNNING)
                        {
                            app_info[i].app_state = APP_STA_RUNNING;
                        }
                        ...
                        {
                            app_info[i].app_state = APP_STA_SUCCESS;
                        }
                        else if (app_info[i].app_result == APP_RET_FAIL)
                        {
                            app_info[i].app_state = APP_STA_ERROR;
            ...
            // 停止应用程序
            if ((app_info[i].app_state == APP_STA_SUCCESS) || (app_info[i].app_state == APP_STA_ERROR))
            {
                if (app_info[i].app_ptr->stop != NULL)
                {
                    app_info[i].app_ptr->stop();
                }
                app_info[i].enable = false;           // 禁用应用程序
                app_info[i].app_state = APP_STA_IDLE; // 设置应用程序状态为空闲
            }
...
void stop_app(APP_NAMES name)
{
    for (int i = 0; i < APP_NUM; i++)
    {
        if (app_table[i].name == name)
        {
            if (app_info[i].app_ptr->stop != NULL)
            {
                app_info[i].app_ptr->stop(); // 停止应用程序
            }
            app_info[i].app_state = APP_STA_IDLE; // 设置应用程序状态为空闲
            app_info[i].enable = false;           // 禁用应用程序
            break;
