...
#define MAVLINK_MESSAGE_INFO_CurrentCtlInput { \
    ...
    {  { "idq_tar_A", NULL, MAVLINK_TYPE_FLOAT, 2, 0, offsetof(mavlink_currentctlinput_t, idq_tar_A) }, \
         ...
         { "mode", NULL, MAVLINK_TYPE_UINT8_T, 0, 48, offsetof(mavlink_currentctlinput_t, mode) }, \
...
static inline uint16_t mavlink_msg_currentctlinput_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               const float *idq_tar_A, const float *iabc_now_A, float elec_theta_rad, float dc_bus_now_V, uint8_t mode, const float *uabc_tar_V, float driver_temp, float elec_angle_speed_rad_s)
{
...
{
    return mavlink_msg_currentctlinput_pack(system_id, component_id, msg, currentctlinput->idq_tar_A, currentctlinput->iabc_now_A, currentctlinput->elec_theta_rad, currentctlinput->dc_bus_now_V, currentctlinput->mode, currentctlinput->uabc_tar_V, currentctlinput->driver_temp, currentctlinput->elec_angle_speed_rad_s);
}
...
{
    return mavlink_msg_currentctlinput_pack_chan(system_id, component_id, chan, msg, currentctlinput->idq_tar_A, currentctlinput->iabc_now_A, currentctlinput->elec_theta_rad, currentctlinput->dc_bus_now_V, currentctlinput->mode, currentctlinput->uabc_tar_V, currentctlinput->driver_temp, currentctlinput->elec_angle_speed_rad_s);
}
...
{
    return mavlink_msg_currentctlinput_pack_status(system_id, component_id, _status, msg,  currentctlinput->idq_tar_A, currentctlinput->iabc_now_A, currentctlinput->elec_theta_rad, currentctlinput->dc_bus_now_V, currentctlinput->mode, currentctlinput->uabc_tar_V, currentctlinput->driver_temp, currentctlinput->elec_angle_speed_rad_s);
}
...
static inline void mavlink_msg_currentctlinput_send(mavlink_channel_t chan, const float *idq_tar_A, const float *iabc_now_A, float elec_theta_rad, float dc_bus_now_V, uint8_t mode, const float *uabc_tar_V, float driver_temp, float elec_angle_speed_rad_s)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_CurrentCtlInput_LEN];
    _mav_put_float(buf, 20, elec_theta_rad);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_currentctlinput_send(chan, currentctlinput->idq_tar_A, currentctlinput->iabc_now_A, currentctlinput->elec_theta_rad, currentctlinput->dc_bus_now_V, currentctlinput->mode, currentctlinput->uabc_tar_V, currentctlinput->driver_temp, currentctlinput->elec_angle_speed_rad_s);
#else
 ...
 */
static inline void mavlink_msg_currentctlinput_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  const float *idq_tar_A, const float *iabc_now_A, float elec_theta_rad, float dc_bus_now_V, uint8_t mode, const float *uabc_tar_V, float driver_temp, float elec_angle_speed_rad_s)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
